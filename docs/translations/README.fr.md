...
Un système de tickets de support complet et intégrable pour Django. Intégrez-le dans n'importe quelle application — obtenez un helpdesk complet avec suivi des SLA, règles d'escalade, workflows des agents et un portail client. Aucun service externe requis.
...
- **Ticket snooze** — Mettre en veille les tickets avec des préréglages (1h, 4h, demain, semaine prochaine) ; la commande `python manage.py wake_snoozed_tickets` les réactive automatiquement
...
- **Real-time broadcasting** — Diffusion optionnelle via Django Channels avec repli automatique sur le polling
...
Les trois modes partagent les mêmes vues, l'interface et la logique métier. Le pattern driver gère le reste.
...
- **[Escalated for Django](https://github.com/escalated-dev/escalated-django)** — Application Django réutilisable (vous êtes ici)
