...
Permissions are upserted via get_or_create on slug.
...
def resolve_permissions(patterns, slug_to_permission):
    """
    ...
    for pattern in patterns:
        if pattern == "*":
            return list(slug_to_permission.values())

        if pattern.endswith(".*"):
            prefix = pattern[:-1]  # e.g. "ticket."
            for slug, perm in slug_to_permission.items():
                if slug.startswith(prefix):
                    resolved.append(perm)
        else:
            if pattern in slug_to_permission:
                resolved.append(slug_to_permission[pattern])
...
class Command(BaseCommand):
    help = "Seed granular permissions and default system roles."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Also reset role-permission assignments for non-system roles.",
        )
    ...
    def _seed_permissions(self):
        for attrs in PERMISSIONS:
            ...
            if not created:
                updated = False
                for field in ("name", "group", "description"):
                    if getattr(permission, field) != attrs[field]:
                        setattr(permission, field, attrs[field])
                        updated = True
                if updated:
                    permission.save()
    ...
    def _seed_roles(self):
        ...
        for definition in ROLES:
            ...
            # Always update attributes for existing system roles
